...
export const useGasStations = () => {
    ...
    async (coordinates: Coordinate[], durationSeconds: number, preferredMinuteMark: number) => {
      ...
      try {
        ...
        if (payload.status !== "OK" && payload.status !== "ZERO_RESULTS") {
          throw new Error(payload.error_message ?? "Gas station search failed");
        }

        const places = Array.isArray(payload.results) ? payload.results : [];

        const nextItems: GasStation[] = places
          .map((place: any) => {
          ...
          .filter((station: GasStation & { distance: number }) => station.distance <= 8000)
          .slice(0, 10)
          .map(({ distance: _distance, ...rest }: GasStation & { distance: number }) => rest);
        ...
        console.warn("RouteDash useGasStations: failed", placesError);
        setState({
          isLoading: false,
          error:
            "We couldn't load gas stations near this route right now. Please try again shortly.",
          items: [],
          targetTravelMinutes: null,
        });
      }
  ...
  return useMemo(
    ...
    [error, fetchGasStations, isLoading, items, reset, targetTravelMinutes],
  );
};
