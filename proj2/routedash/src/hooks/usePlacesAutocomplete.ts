...
export const usePlacesAutocomplete = () => {
  const [{ isLoading, error, suggestions }, setState] =
    ...
    async (input: string) => {
      ...
      if (!key) {
        setState({
          isLoading: false,
          error:
            "Missing Google Maps API key. Add GOOGLE_MAPS_API_KEY to your .env file.",
          suggestions: [],
        });
        return [];
      }
      ...
      try {
        ...
        if (payload.status !== "OK" || !Array.isArray(payload.predictions)) {
          throw new Error(
            payload.error_message ?? "Autocomplete request failed",
          );
        }

        const mapped: PlaceSuggestion[] = payload.predictions.map(
          (prediction: any) => ({
        ...
        console.warn(
          "RouteDash usePlacesAutocomplete: failed fetching suggestions",
          autoError,
        );
        setState({
          isLoading: false,
