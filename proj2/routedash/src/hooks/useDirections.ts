    ...
    async (origin: string, destination: string) => {
      ...
      if (!inputOrigin || !inputDestination) {
        setState((prev) => ({
          ...prev,
          error:
            "Enter both a starting point and a destination to plot your trip.",
        }));
        return false;
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorPayload.error?.message ?? "Directions request failed",
          );
        }
        ...
        const parseDurationSeconds = (duration: any): number => {
          ...
          if (typeof duration === "string") {
            const normalized = duration.endsWith("s")
              ? duration.slice(0, -1)
              : duration;
        ...
        console.warn(
          "RouteDash useDirections: failed to fetch directions",
          routeError,
        );
        setState({
          isLoading: false,
