...
describe("services/restaurantService", () => {
  ...
  it("updates existing menu sections", async () => {
    prisma.menuSection.findUnique.mockResolvedValue({
      id: "sec-1",
      title: "Breakfast",
      position: 0,
    } as any);
    prisma.menuSection.update.mockResolvedValue({
      id: "sec-1",
      title: "Brunch",
      position: 1,
    } as any);
  ...
  it("throws when deleting missing menu item", async () => {
    prisma.menuItem.findFirst.mockResolvedValue(null);
    await expect(deleteMenuItem("rest-1", "item-1", "user-1")).rejects.toMatchObject({
      status: 404,
    });
  });
